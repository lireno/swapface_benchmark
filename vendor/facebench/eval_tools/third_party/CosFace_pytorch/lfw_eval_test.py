...
if __name__ == '__main__':
    ...
    # _, result = eval(net.sphere().to('cuda'), model_path='checkpoints/ACC99.28.pth')
    # np.savetxt("result.txt", result, '%s')

    model = net.sphere().to('cuda:0')
    model.load_state_dict(torch.load("checkpoints/ACC99.28.pth"))
    model.eval()

    with open("/mnt/nas/share2/home/lzk/codes/swap_face/eval_datas/tools/test1.png", 'rb') as f:
        img1 =  Image.open(f).convert('RGB')
    with open("/mnt/nas/share2/home/lzk/codes/swap_face/eval_datas/tools/test2.png", 'rb') as f:
        img2 =  Image.open(f).convert('RGB')
    f1 = extractDeepFeature(img1, model, is_gray=False)
    f2 = extractDeepFeature(img2, model, is_gray=False)
