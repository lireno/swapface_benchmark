...
# def align_img(img, lm, lm3D, mask=None, target_size=224., rescale_factor=102., frame_idx=None):
#     """
#     Return:
...
#     # 原始图像尺寸
#     w0, h0 = img.size
#     print(prefix + f"输入图像尺寸: (W={w0}, H={h0}), landmarks shape={lm.shape}, lm3D shape={lm3D.shape}")

#     # landmarks 检查
#     if lm.shape[0] != 5:
#         print(prefix + f"landmarks 不是 5 点，而是 {lm.shape[0]} 点，调用 extract_5p()")
#         lm5p = extract_5p(lm)
#     else:
#         print(prefix + "landmarks 已经是 5 点")
#         lm5p = lm
#     print(prefix + f"5点 landmarks: {lm5p}")

#     # 计算仿射变换参数
#     try:
#         t, s = POS(lm5p.transpose(), lm3D.transpose())
#         print(prefix + f"POS 返回: t={t}, s={s}")
#     except Exception as e:
#         print(prefix + f"POS 计算失败: {e}")
#         return None, None, None, None

#     s = rescale_factor / s
#     print(prefix + f"缩放因子调整后: s={s}")
...
#         )
#         print(prefix + f"resize_n_crop_img 成功: 新图像尺寸={img_new.size}, 新 landmarks shape={lm_new.shape}, "
#               f"mask_new={'None' if mask_new is None else mask_new.size}")
#     except Exception as e:
#         print(prefix + f"resize_n_crop_img 出错: {e}")
#         return None, None, None, None

#     # 保存变换参数
#     trans_params = np.array([w0, h0, s, t[0], t[1]])
#     print(prefix + f"变换参数: {trans_params}")
