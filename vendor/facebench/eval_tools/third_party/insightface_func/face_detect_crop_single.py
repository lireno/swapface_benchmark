...
def _detect_faces(det_model, img, det_thresh, max_num):
    try:
        return det_model.detect(
            img,
            threshold=det_thresh,  # insightface==0.2.1
            max_num=max_num,
            metric='default',
        )
...
class Face_detect_crop:
    ...
    def get(self, img, crop_size, max_num=0):
        # self.det_thresh = 0.0
        bboxes, kpss = _detect_faces(self.det_model, img, self.det_thresh, max_num)
        ...
        align_img = cv2.warpAffine(img, M, crop_size, borderValue=0.0)
